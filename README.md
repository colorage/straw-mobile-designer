...
- **Rigid loops**: when ties close a rigid cluster — hand-tied straw loops, triangle corners / tetrahedra built from triangle primitives, or two pieces locked at three or more corners — the cluster is fused into one rigid piece so it hangs as steadily as a single prebuilt body instead of wobbling on soft thread joints. Edge-to-edge ties at only two corners stay hinged (a chain of squares hung one-by-one, or a ring of squares, remains floppy). A simple straw loop also snaps to its regular shape on fusing — four equal straws become a true square rather than a frozen parallelogram, two solid plus two 1/2 straws a rectangle, five equal straws a regular pentagon, and so on (braced or 3D builds keep the shape you gave them). Mixed-size pieces remember each straw's size for the inventory and scissors; once fused, scissors cuts one straw at a time (including edges that came from a swallowed primitive). Ceiling-hook ties and single-thread bridges stay floppy ball joints so the mobile still swings. Toggle with the braced-frame button (top-right); switching it back on also stiffens loops you already built, and undo un-fuses.
- **Undo / Redo**: reverse or re-apply design edits (add/remove shapes, connections, moves, straw size) via the Project panel or Ctrl/Cmd+Z / Ctrl/Cmd+Shift+Z. History is per project — loading another gallery entry or starting New clears it so Undo cannot pull shapes from a different mobile.
- **In-app help**: an empty-canvas tip appears when the scene has no shapes; press **?** or the top-right help button for keyboard shortcuts and build tips.
- **Gallery** (`/gallery`): your **Projects** and the **Community** gallery on one screen. Named local or cloud-saved projects with thumbnails; the current project is saved automatically on every edit and when opening the gallery. Browse/load/import/export/delete in Projects, plus **New** to clear the draft and start fresh in the designer. Signed-in users can **Publish** a project to the community; once published, further edits re-sync automatically. Below that, Community lists published mobiles (sort by **Recent** or **Most liked**), likes (signed-in), and opens a read-only **preview** (`/community/:id`) with orbit + physics. From preview: like, **comment** (signed-in; optional photos), and **Duplicate to my gallery** to open a local/cloud copy in the full editor. `/community` redirects to `/gallery#community`.
