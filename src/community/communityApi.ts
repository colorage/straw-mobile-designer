...
const LIST_COLUMNS = 'id, name, thumbnail_data_url, likes_count, published_at, owner'
...
export async function fetchCommunityProjects(
  ...
  let query = getSupabase().from('public_projects').select(LIST_COLUMNS)
  query =
    sort === 'liked'
      ? query
