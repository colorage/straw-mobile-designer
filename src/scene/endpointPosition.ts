...
export function getEndpointWorldPosition(
  ...
  const reelPosition = store.reelPositions[endpoint.shapeId]
  const reelQuaternion = store.reelQuaternions[endpoint.shapeId]
  const body = getBodyRef(endpoint.shapeId).current
  // Prefer live reel pose so the thread shortens in sync with the sliding mesh.
  if (reelPosition) {
    const quat = reelQuaternion ?? shape.quaternion
    return new THREE.Vector3(lx, ly, lz)
      .applyQuaternion(new THREE.Quaternion(...quat))
